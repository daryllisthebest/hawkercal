...
const PRO_SCAN_KEY = 'hawkercal_pro_scans'
const CUSTOM_DISHES_KEY = 'hawkercal_custom_dishes'
const CORRECTIONS_KEY = 'hawkercal_corrections'

export const FREE_SCAN_LIMIT = 3
export const PRO_MONTHLY_SCAN_LIMIT = 100
...
export function getCustomDishes() {
  return safeGet(CUSTOM_DISHES_KEY, [])
}

export function addCustomDish({ name, emoji, calories, description, origin }) {
  const id = 'custom-' + name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '') + '-' + Date.now()
  const dish = {
    id,
    name: name.trim(),
    emoji: emoji || '🍽️',
    origin: origin?.trim() || 'Custom',
    flag: '🌍',
    baseCalories: Math.max(0, parseInt(calories) || 0),
    description: description?.trim() || '',
    tags: ['Custom'],
    macros: { protein: 0, carbs: 0, fat: 0, fiber: 0 },
    isCustom: true,
  }
  safeSet(CUSTOM_DISHES_KEY, [...getCustomDishes(), dish])
  return dish
}

export function removeCustomDish(id) {
  safeSet(CUSTOM_DISHES_KEY, getCustomDishes().filter(d => d.id !== id))
}

export function getCorrections() {
  return safeGet(CORRECTIONS_KEY, [])
}

export function addCorrection({ originalDishId, selectedDishId, selectedDishName, ingredients, confidence, stage1, stage2 }) {
  ...
  const correction = {
    ...
    userId: 'local',
    originalDishId,
    selectedDishId: selectedDishId || null,
    selectedDishName,
  ...
  safeSet(CORRECTIONS_KEY, [...corrections, correction])
  return correction
}
...
export function getMonthlyProScanCount() {
  const data = safeGet(PRO_SCAN_KEY, {})
  return data.month === getMonthStr() ? (data.count || 0) : 0
}

export function incrementMonthlyProScanCount() {
  const month = getMonthStr()
  const data = safeGet(PRO_SCAN_KEY, {})
  const count = data.month === month ? (data.count || 0) + 1 : 1
  safeSet(PRO_SCAN_KEY, { month, count })
  return count
}
